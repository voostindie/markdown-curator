...
public interface Query
{
    ...
    /// Returns whether the changelog impacts this query definition; i.e., might produce different
    /// output than is currently in the document. When unsure, reply `true`.
    ///
    ...
    ///
    /// Returning `false` does not mean that the query will not be executed. If the document does
    /// not contain an output hash for the definition, or if there is another change in the document
    /// that has an impact, then the query will be executed anyway.
    ///
