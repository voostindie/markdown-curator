package nl.ulso.curator.query;

import jakarta.inject.Inject;

import java.util.Map;
import java.util.function.Predicate;

import static java.lang.System.lineSeparator;
import static java.util.regex.Pattern.compile;
import static nl.ulso.curator.query.OutputFormat.MARKDOWN;

final class TableMarkdownFormatter
    implements QueryResultFormatter<TableResult>
{
    private static final char NORMAL_HYPHEN = '-';
    private static final char NON_BREAKING_HYPHEN = '‑';
    private static final Predicate<String> DATE_PREDICATE =
        compile("^\\d{4}-\\d{2}-\\d{2}$").asPredicate();

    @Inject
    public TableMarkdownFormatter()
    {
    }

    @Override
    public Class<TableResult> queryResultType()
    {
        return TableResult.class;
    }

    @Override
    public OutputFormat outputFormat()
    {
        return MARKDOWN;
    }

    @Override
    public String format(TableResult result)
    {
        var widths = findColumnWidths(result);
        var width = result.columns().size();
        var builder = new StringBuilder();
        builder.append(lineSeparator()).append("|");
        for (var i = 0; i < width; i++)
        {
            builder.append(" ");
            var column = capitalize(result.columns().get(i));
            builder.append(column);
            builder.repeat(" ", Math.max(0, widths[i] - column.length()));
            builder.append(" |");
        }
        builder.append(lineSeparator());
        builder.append("|");
        for (var i = 0; i < width; i++)
        {
            builder.append(" ");
            switch (result.alignments().get(i))
            {
                case LEFT:
                    builder.repeat("-", Math.max(widths[i], 1));
                    break;
                case RIGHT:
                    builder.repeat("-", Math.max(widths[i] - 1, 1));
                    builder.append(":");
                    break;
                case CENTER:
                    builder.append(":");
                    builder.repeat("-", Math.max(widths[i] - 2, 1));
                    builder.append(":");
                    break;
            }
            builder.append(" |");
        }
        builder.append(lineSeparator());
        for (Map<String, String> row : result.rows())
        {
            builder.append("|");
            for (var i = 0; i < width; i++)
            {
                builder.append(" ");
                var column = applyObsidianFormattingWorkaround(
                    row.getOrDefault(result.columns().get(i), ""));
                builder.append(column);
                builder.repeat(" ", Math.max(0, widths[i] - column.length()));
                builder.append(" |");
            }
            builder.append(lineSeparator());
        }
        builder.append(lineSeparator());
        return builder.toString();
    }

    /*
     * The table formatting in Obsidian is such that it wraps along hyphens, which is correct most
     * of the time, except when the column is a date. In that case the dates might be wrapped. To
     * prevent dates being wrapped this method replaces the normal hyphens with non-breaking
     * hyphens. They look exactly the same visually, which is great when looking at the table
     * in Markdown format.
     *
     * I'm aware that this workaround only applies if dates are formatted as "YYYY-MM-DD", which
     * is what I do. For people formatting dates differently, this workaround simply doesn't work.
     */
    private String applyObsidianFormattingWorkaround(String column)
    {
        if (DATE_PREDICATE.test(column))
        {
            return column.replace(NORMAL_HYPHEN, NON_BREAKING_HYPHEN);
        }
        return column;
    }

    private String capitalize(String string)
    {
        var length = string.length();
        if (length == 0)
        {
            return string;
        }
        if (length == 1)
        {
            return string.toUpperCase();
        }
        return string.substring(0, 1).toUpperCase() + string.substring(1);
    }

    private int[] findColumnWidths(TableResult result)
    {
        var width = result.columns().size();
        var widths = new int[width];
        for (var i = 0; i < width; i++)
        {
            widths[i] = result.columns().get(i).length();
        }
        for (Map<String, String> row : result.rows())
        {
            for (var i = 0; i < width - 1; i++)
            {
                widths[i] = Math.max(
                    widths[i],
                    row.getOrDefault(result.columns().get(i), "").length()
                );
            }
        }
        return widths;
    }
}
